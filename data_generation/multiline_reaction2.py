...
def generate_reaction_image_multi2(reactants_smiles, products_smiles, condition):
    ...
    cv2.arrowedLine(full_img, arrow_start, arrow_end, (0, 0, 0), thickness=random.randint(2, 5), tipLength=0.05)
    # 当前位置
    x_offset += 220 + random.randint(-10, 10)  # 留出空间给箭头
...
def generate_json_from_multiple_reactions_with_render2(i, title, results1):
    # Initialize the main dictionary structure
    output_dict = {
        "id": i + 32000,
        "width": 1333,
        "height": 450 * len(results1),
        "file_name": f"{title}.png",
        "license": 0,
        "bboxes": [],
        "reactions": [],
    ...
    for index, result in enumerate(results1):
        conditions_ids = []
        # Conditions
        for condition_type, condition_bbox in result['conditions'].items():
        ...
        if index < len(results1) - 1:  # Ensure we aren't on the last reaction
            output_dict['reactions'].append({
...
if __name__ == "__main__":
    ...
    for year in range(2001,2005):  # 从2001到2023
        # 构建文件路径的模式
        pattern = f"pistachio/data/extract/applications/2001/*_txt_reactions_wbib.json"
        # 使用glob查找匹配的文件
    ...
    output_folder = "generated_images_debug_single"
    ...
    for index, reaction in enumerate(results):
        ...
        try:
            ...
            cv2.imwrite(combined_filename, image)
                    #box_img = draw_boxes_on_reaction_image(combined_image, updated_coordinates)
                    #combined_filename1 = os.path.join(output_folder, f"box_{title}.png")
                    #cv2.imwrite(combined_filename1, box_img)
                    

            output_dict = generate_json_from_multiple_reactions_with_render2(index, title, coordinates)
    ...
    print(output_dicts)
    save_to_json(output_dicts, 'outputdebug_multiline2.json')